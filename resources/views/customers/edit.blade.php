            ...
            <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="4"
                              class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">{{ old('notes', $customer->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-between">
                    <a href="{{ route('customers.show', $customer) }}" class="text-blue-500 hover:underline">Cancel</a>
                    <button type="submit"
